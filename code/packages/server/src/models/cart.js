module.exports = function Cart(oldCart){ // oldCart là cái cart cũ 
    ...
    this.add = function(item,id){  // thực hiện thêm item vào trong cart , item này có thể đã có trước trong cart 
        var storeItem = this.items[id];  //storeItem chính là đối tượng sản phẩm 
        if(!storeItem){
            storeItem = this.items[id] = {item:item , qty:0,price:0};  // nếu item chưa có trong cart thì bắt đầu tạo mới item này trong cart ... price trong đối tượng storeItem là tổng giá của storeItem
        }
        storeItem.qty++;
